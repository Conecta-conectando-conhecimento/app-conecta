...
function Register() {
  ...
  const handleAddUser = async (e) => { //email: string, cpf: string, name: string, user_name: string, birthday: Date, password: string
    const userData = {
      email: email,
      cpf: cpf,
      name: nomeCompleto,
    ...
    try {
      const response = await AuthController.registerUser(userData);
